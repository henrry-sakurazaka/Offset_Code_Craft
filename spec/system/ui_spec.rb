RSpec.describe 'topページ', type: :system do
  it 'タイトルが表示される' do
    visit '/'
    expect(page).to have_css('.catch', text: /Where Imagination\s*Meets Possibility\s*Unleashing the Power of Innovation/i)
  ...
  it 'オーディオ装置のアイコン、画像が表示されている' do
    ...
    expect(page).to have_css("[style]", text: /background-image:\s*url\(.*images_mountain\/2\.jpg.*\)/)
